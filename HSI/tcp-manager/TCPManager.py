    ...
    def _listenTCPHelper(self, connection, client_address):
        ...
        try:
            ...
            if data != []:
                ...
                data_arr = ""
                try:
                    data_arr = data[0].decode().strip()
                except Exception as ex:
                    data_arr = "old"
                if(data_arr == "old"):
                    data_arr = ComparableDict(pickle.loads(b"".join(data)))
                else:
                    dataList = data_arr.split(";")
                    data_arr = ComparableDict({"id": int(float(dataList[0])),
                                "cmd": dataList[1],
                                "priority": int(dataList[2]),
                                "leftSpeed": int(dataList[3]),
                                "rightSpeed": int(dataList[4]),
                                "receivingPort": int(dataList[5])
                                })
