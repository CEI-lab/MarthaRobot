...
out = cv2.VideoWriter('choice.avi', cv2.VideoWriter_fourcc('M','J','P','G'),6.1,(640,480))

cv2.namedWindow("window", cv2.WINDOW_FREERATIO)
cv2.setWindowProperty("window",cv2.WND_PROP_FULLSCREEN,cv2.WINDOW_FULLSCREEN)

size =  250
ret, sad = cv2.threshold(cv2.cvtColor(cv2.resize(cv2.imread('sad.png'),(size,size))  , cv2.COLOR_BGR2GRAY),127, 255, cv2.THRESH_BINARY_INV) 
ret, hap = cv2.threshold(cv2.cvtColor(cv2.resize(cv2.imread('happy.png'),(size,size)), cv2.COLOR_BGR2GRAY),127, 255, cv2.THRESH_BINARY_INV) 
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    circle_mask = cv2.circle(circle_mask,(cam_c[0],cam_c[1]),cam_c[2],(255,255,255),cv2.FILLED)
    ...
    if largest_con:
        ...
        if center[0] < pro_c[0]:
            dot_img[pro_c[1]-int(size/2):pro_c[1]+int(size/2),pro_c[0]-int(size/2):pro_c[0]+int(size/2)] = sad
        else:
            dot_img[pro_c[1]-int(size/2):pro_c[1]+int(size/2),pro_c[0]-int(size/2):pro_c[0]+int(size/2)] = hap


    else:
        dot_img = np.zeros((959,1706))
        dot_img[pro_c[1]-int(size):pro_c[1],pro_c[0]-int(size/2):pro_c[0]+int(size/2)] = sad
        dot_img[pro_c[1]:pro_c[1]+int(size),pro_c[0]-int(size/2):pro_c[0]+int(size/2)] = hap
