...
class SetSpeedCommand(CommandInterface):
    ...
    def __init__(self):
        try:
            self._motor_left = SpeedController(CONFIGURATIONS.get("LEFT_WHEEL_SPEED_CONTROLLER_SERIAL_ID"))
            self._motor_right = SpeedController(CONFIGURATIONS.get("RIGHT_WHEEL_SPEED_CONTROLLER_SERIAL_ID"))
        except:
        ...
        try:
            ...
            if left_speed is not None and right_speed is not None:
                ...
                if left_speed is not None:
                    logging.info("SetSpeedCommand : Set left speed: " + str(left_speed) + ".")
                    jsonObject["response"] = "SUCCESS"
                    self._motor_left.set_speed(left_speed)
                if right_speed is not None:
                    logging.info("SetSpeedCommand : Set right speed: " + str(right_speed) + ".")
                    jsonObject["response"] = "SUCCESS"
