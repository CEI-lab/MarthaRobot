...
def compensate_x(mag_data_x, data_rhall, trim_data):
    ...
    if ((mag_data_x != -4096) and (data_rhall != 0) and (trim_data['xyz1'] != 0)):
        ...
        process_comp_x2 = process_comp_x1 + \
            retval * (trim_data['xy1']) / 16384.0
        process_comp_x3 = trim_data['x2'] + 160.0
        process_comp_x4 = mag_data_x * \
            ((process_comp_x2 + 256.0) * process_comp_x3)
...
def configure_bmi150():
    ...
    bus = smbus.SMBus(2)
    # Set Power Control bit to en
    ret = bus.write_byte_data(address, Aux_Control, 0x01)
    time.sleep(0.05)
    # Set Operation mode to Normal
    ret = bus.write_byte_data(address, Aux_Op_Mode, 0x00)
    time.sleep(0.05)
