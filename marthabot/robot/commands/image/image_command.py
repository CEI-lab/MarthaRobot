...
class ImageCommand(CommandInterface):
    def ExecuteHelper(self, responseStatusCallback, jsonObject):
        try:
            ...
            if (
                jsonObject["type"] == "upload"
                and "data" in jsonObject
                and "name" in jsonObject
            ):
                cv2.imwrite("images/{}".format(jsonObject["name"]), jsonObject["data"])
            if (
                jsonObject["type"] == "display"
                and "data" in jsonObject
                and "name" not in jsonObject
            ):
                config.DISPLAY_IMAGE = jsonObject["data"]
            if (
                jsonObject["type"] == "display"
                and "name" in jsonObject
                and "data" not in jsonObject
            ):
...
if __name__ == "__main__":
    ...
    obj.execute(
        None,
        {
            "cmd": "ProcessProjectImageCommand",
            "imageFileName": "a.png",
            "processImage": True,
            "projectImage": True,
            "priority": 1,
