...
class StreamingServer(asyncore.dispatcher):
    def __init__(self, address):
        ...
        try:
            ...
            time.sleep(0.5)
            self.tof_objects = {}
            self.tof_objects[self.shutdown_pins[0]
                             ] = VL53L0X.VL53L0X(address=0x2A)
            self.tof_objects[self.shutdown_pins[1]
                             ] = VL53L0X.VL53L0X(address=0x2C)
            self.tof_objects[self.shutdown_pins[2]
                             ] = VL53L0X.VL53L0X(address=0x2E)
            for pin in self.shutdown_pins:
                GPIO.output(pin, GPIO.HIGH)
                time.sleep(0.50)
                self.tof_objects[pin].start_ranging(
