...
class RealSenseCommand(CommandInterface):
        ...
        try:
            if jsonObject["type"] == "single":
                ...
                if "colorize" in jsonObject and jsonObject["colorize"] == 1:
                    np_image = cv2.applyColorMap(cv2.convertScaleAbs(np_image, alpha=0.05), cv2.COLORMAP_JET)
                jsonObject["data"] = np_image
            if jsonObject["type"] == "stream":
                server = RealSenseServer.RSMulticastServer()
                self.stream = True
                asyncore.loop()
        except:
    ...
    def execute(self, responseStatusCallback, jsonObject):
        if (jsonObject["type"] == "stream" and self.streaming == False) or jsonObject["type"] == "single":
