...
port = CONFIGURATIONS["INT_CAM_PORT"]
chunk_size = 4096
           
class IntStreamingServer(asyncore.dispatcher):
    ...
    def __init__(self, address, jsonObject):
        ...
        try:
            logging.info("Launching Int Camera Server")
            ...
            logging.error("Unexpected error: Internal Camera Streaming Server")
...
#         self.camera = cv2.VideoCapture(self.dev)
#         logging.info(str(self.width) + " " + str(self.height))
...
class IntCamMulticastServer(asyncore.dispatcher):
    ...
    def handle_read(self):
        try:
           ...
           # Once the server recives the multicast signal, open the frame server
            self.server = IntStreamingServer(addr, self.jsonObject)
        except:
            logging.error("?RS")
